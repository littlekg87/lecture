...
| ② 단어 분석 | 상소문의 단어 빈도·워드클라우드 | Python (구글 코랩) |
| ③ 통계 분석 | 호구 수와 군사 수의 상관관계 | jamovi |
| ④ GIS | 팔도별 통계를 지도로 그리기 | QGIS |
...
| 한글 폰트 | 맑은 고딕 | **맑은 고딕이 없음** → 코랩이 해결 |
| `.hwp` 파일 | 한글에서 열림 | 열기 어려움 → **`.txt` 파일을 쓰세요** |
...
2. **Gephi 이름표가 `□□□` 네모로 나오는 문제**
   ...
   자세한 내용은 [Gephi 가이드](guides/gephi.md#3-이름-보이게-하기--️-맥-사용자-필독) 참고.
...
각 노트북 마지막의 **"내려받기"** 칸을 실행해 내 컴퓨터로 저장하세요.
